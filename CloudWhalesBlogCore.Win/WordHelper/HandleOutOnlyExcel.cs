...
{
    public class HandleOutOnlyExcel : WordHandleSuper
    {
        public HandleOutOnlyExcel(string excelPath) : base(excelPath)
        {
        }
                    ...
                    try
                    {
                        ...
                        foreach (var wordContent in wordContentList)
                        {
                            ...
                            foreach (var item in contentArray)
                            {
                                ...
                                {
                                    progressbar.SetInfo(null, $"共{contentArray.Length}项，已执行{i - 1}项", $"当前正在执行：{i}");
                                }));
                                Thread.Sleep(2);
                                progressbar.TryBeginInvoke(new Action(() =>
                                {
                                    progressbar.SetProgress(i, contentArray.Length);
                                }));
                            }
                        }
                    }
