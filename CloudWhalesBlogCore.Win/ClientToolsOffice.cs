...
namespace CloudWhalesBlogCore.Win
{
    public partial class ClientToolsOffice : Form
    {
        ...
        {
            var photoItem = await Task.Run(() => ExcelHelper.SaveExcelPhotos(BtnOpenFile.Text, BtnOutputDirectory.Text));
            if (photoItem != null)
            {
        ...
        {
            ExcelHelper excelHelper = new(BtnOpenFile.Text);
            var sheetItem = await Task.Run(() => excelHelper.GetSaveAsExcel(BtnOutputDirectory.Text));
            if (sheetItem != null)
            {
        ...
        {
            ExcelHelper excelHelper = new(BtnMergeDocument.Text);
            var mergeExcel = excelHelper.MergeExcel(BtnMergeOutput.Text);
        ...
        private async void BtnWordPhotos_Click(object sender, EventArgs e)
        {
            ...
            using ExcelHelper excelHelper = new(BtnOpenFile.Text);
            using WordHelper wordHelper = new(BtnOpenFile.Text);
